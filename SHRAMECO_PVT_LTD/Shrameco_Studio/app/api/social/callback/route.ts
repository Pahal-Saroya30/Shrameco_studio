...
export async function GET(req: NextRequest) {
	try {
		...
		// Try to extract returnTo from state so we redirect back to the right page with the error
		...
		try {
			...
			if (stateParam) {
				const pending = verifyOAuthState(stateParam);
				if (pending?.returnTo) errorReturnTo = pending.returnTo;
				if (pending?.platform) errorReturnTo = `/dashboard/${pending.platform}`;
			}
		} catch (_) {}

		return redirectWithError(req, (err as Error).message || 'Failed to connect account.', errorReturnTo);
	}
}
