...
export default function OnboardingPage() {
	...
	const cta = watch('cta');
	const socialHandle = watch('socialHandle');
		...
		async function fetchBrandProfile() {
			try {
				...
				if (res.ok && data.profile) {
					...
					setValue('logoPosition', p.logoPosition || 'top-right');
				}
			} catch (err) {
	...
	return (
		<div className="min-h-screen bg-[#EFF6F7] flex flex-col font-sans selection:bg-[#3D8090]/20 selection:text-[#2D5F68] relative overflow-hidden">
			<Navbar />

			{/* Floating Background Glow Blobs */}
			<div className="absolute top-1/4 left-[-10%] w-[500px] h-[500px] bg-indigo-400/10 rounded-full blur-[120px] pointer-events-none animate-pulse" style={{ animationDuration: '6s' }} />
			<div className="absolute bottom-1/4 right-[-10%] w-[500px] h-[500px] bg-[#3D8090]/10 rounded-full blur-[120px] pointer-events-none animate-pulse" style={{ animationDuration: '8s' }} />
					...
					>
						...
						{step === 1 && (
							<div className="space-y-6 animate-fadeIn">
								...
								<div className="pt-4 border-t border-[#B8D4D8]/30 space-y-4">
									<h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Marketing & Outro Context</h3>
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
										<Input
											label="Brand Tagline / Slogan"
											placeholder="e.g. Automate your posts"
											error={errors.slogan?.message}
											{...register('slogan')}
										/>
										<Input
											label="Default Outro Link / CTA"
											placeholder="e.g. acme.com/try"
											error={errors.cta?.message}
											{...register('cta')}
										/>
										<Input
											label="Default Social Handle / Watermark"
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
										{[
											...
											<button
												key={p.id}
												type="button"
												onClick={() => setValue('logoPosition', p.id)}
												className={`py-2 px-3 rounded-2xl text-xs font-bold border transition-all text-center ${
													logoPosition === p.id 
														? 'bg-[#3D8090] border-[#3D8090] text-white shadow-md' 
														: 'bg-white border-[#B8D4D8]/50 text-slate-650 hover:bg-slate-50'
												}`}
											>
												{p.label}
											</button>
									...
									<div className="space-y-2 pt-1">
										...
										<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
											{[
												{
													name: '⚡ Neon Tech',
													colors: ['#4F46E5', '#10B981', '#06B6D4', '#F59E0B']
												},
												{
													name: '🌇 Sunset Rose',
													colors: ['#EC4899', '#F43F5E', '#E11D48', '#FFEDD5']
												},
												{
													name: '🌲 Royal Mint',
													colors: ['#047857', '#059669', '#10B981', '#ECFDF5']
												},
												{
													name: '🌊 Deep Slate',
													colors: ['#0F172A', '#2563EB', '#38BDF8', '#E0F2FE']
												}
											].map((theme) => (
												<button
													key={theme.name}
													type="button"
													onClick={() => setValue('colorPalette', theme.colors)}
													className="p-2.5 rounded-2xl bg-white border border-[#B8D4D8]/50 hover:border-[#3D8090] transition-all text-left space-y-1.5 cursor-pointer shadow-xs hover:shadow-sm"
												>
													<span className="block text-[10px] font-extrabold text-slate-700">{theme.name}</span>
													<div className="flex space-x-1">
														{theme.colors.slice(0, 3).map((c, i) => (
															<div key={i} className="w-4 h-4 rounded-md border border-slate-900/5" style={{ backgroundColor: c }} />
														))}
													</div>
												</button>
											))}
